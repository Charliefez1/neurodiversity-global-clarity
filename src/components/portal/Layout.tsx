      ...
      <div className="min-h-screen flex flex-col">
        <header className="bg-card border-b px-4 md:px-6 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileOpen(true)} className="md:hidden p-1.5 -ml-1 rounded-md hover:bg-secondary" aria-label="Open menu">
            ...
            <span className="hidden lg:block text-sm font-bold text-foreground">
              {client?.name || 'Neurodiversity Global'} Client Portal
            </span>
          </div>
          <div className="flex items-center gap-2 md:gap-3">
            <span className={`text-xs font-semibold px-2 md:px-3 py-1 rounded-full ${
              isAdmin ? 'bg-destructive/10 text-destructive border border-destructive/20' : 'bg-primary/10 text-primary border border-primary/20'
            }`}>
            ...
            <Link to="/" className="text-xs text-muted-foreground hover:text-foreground hidden sm:inline-flex items-center gap-1">
              <Home className="h-3 w-3" /> Main site
            </Link>
          </div>
        </header>
        ...
        <div className="flex flex-1">
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            <SheetContent side="left" className="w-72 p-0">
              ...
              <div className="p-2 border-t space-y-1">
                <button onClick={() => { logout(); navigate('/portal/login'); setMobileOpen(false); }}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm text-muted-foreground hover:bg-secondary w-full">
                  <LogOut className="h-4 w-4" /> Log out
                </button>
              </div>
            ...
            {!isHome && (() => {
              ...
              return (
                <nav className="flex items-center gap-1.5 text-sm mb-5">
                  <Link to={homePath} className="inline-flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors">
